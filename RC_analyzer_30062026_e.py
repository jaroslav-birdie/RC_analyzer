...
# =============================================================================
        ...
        # the heavy 3D rotation is done once per segmentation, the playback only cycles ready images
        self.mip_frames = []   # list of PIL images, one per rotation angle
        self.mip_index = 0     # currently displayed rotation frame
        self.mip_n_frames = 36 # number of frames per full 360 deg turn (10 deg step)
        self.mip_downsample = 2 # in-plane downsampling factor used before rotating (speeds up the precompute)
...
# =============================================================================
    ...
    def build_mip_frames(self, of_what):
        # variant B: precompute the rotating front-view MIP once (called after each source segmentation).
        ...
        for k in range(self.mip_n_frames):
            ...
            frames.append(img)
        self.mip_frames = frames
        if self.mip_index >= len(frames):
            self.mip_index = 0
...
class mainWindow:
    ...
    def rois_pos_save(self, i):
        ...
        # rebuild the rotating MIP frames right after this source is segmented (variant B) and start spinning -
        # the view rotates as soon as a single source has been segmented
        self.phantom.build_mip_frames(self.phantom.mask_labelled)
    ...
    def spin_step(self):
        if not self.spin_running:
            return
        if self.phantom.mip_frames:
            self.phantom.mip_index = (self.phantom.mip_index + 1) % len(self.phantom.mip_frames)
            self.phantom.show_mip_frame(self.mip_view, self.phantom.mip_position, self.phantom.mask_zoom)
        self.spin_after_id = self.master.after(self.spin_interval, self.spin_step)

    def start_spin(self):
        # begin (or keep) the rotation - called automatically after a source is segmented
        if not self.spin_running:
