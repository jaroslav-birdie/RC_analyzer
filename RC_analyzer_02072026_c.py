...
#
# 02.07.2026    - CRC calculation added based on Sunderland JJ, J Nucl Med 2025; 00:1–10
...
#               - for batch processing of the same data recorded multiple times
#                   - lesion diameters are kept when opening another dataset
...
# =============================================================================
        ...
        self.data = data
        self.data_max = np.max(data) if np.size(data) else 1 # cached once - reused on every slice redraw
        ...
        # RC (recovery coefficient) dictionaries 
        ...
        # CRC (contrast recovery coefficient) dictionaries (Sunderland et al., J Nucl Med)
        ...
        # rotating MIP using precomputed rotation frames:
        # the 3D rotation runs once per segmentation on a background thread, the playback
        # only cycles the precomputed images
        self.mip_frames = []   # list of PIL images, one per rotation angle
        self.mip_index = 0     # currently displayed rotation frame
        self.mip_n_frames = 24 # number of frames per full 360 deg turn (15 deg step) - compromise between visual / performace
        # adaptive in-plane downsampling for better performace
        inplane = max(self.size[0], self.size[1])
        if inplane >= 400:
            self.mip_downsample = 3
        elif inplane >= 200:
            self.mip_downsample = 2
        else:
            self.mip_downsample = 1
        self.mip_build_token = 0  # generation id - lets a newer build invalidate a still running older one
        self.mip_ready_token = -1 # token of the frame set currently stored in mip_frames

# =============================================================================
# object functions
# =============================================================================
    def show_me_slices(self, of_what, which, where, position, zoom, thresh):
        # use the cached volume maximum (self.data_max) instead of scanning the whole volume on every redraw
        temp = Image.fromarray(cm.Greys(of_what[:,:,which]/(self.data_max/thresh), bytes=True))
    ...
    def compute_mip_frames(self, of_what):
        # builds the rotating front-view MIP frames
        # The volume is rotated around its long axis in mip_n_frames steps;
        # each rotated volume is summed along axis 0 to give the side/front projection
        ...
        for k in range(self.mip_n_frames):
            angle = k * (360.0 / self.mip_n_frames)
            rot = vol if angle == 0 else ndi_rotate(vol, angle, axes=(0, 1), reshape=False, order=0)
            proj = np.rot90(np.sum(rot, axis=0), 1)
            peak = np.max(proj)
            if peak != 0:
                img = Image.fromarray(cm.Greys(proj / peak, bytes=True)) # division by 0 exemption
            else:
                img = Image.fromarray(cm.Greys(proj, bytes=True))
            frames.append(img)
        return frames
    ...
    def show_mip_frame(self, where, position, zoom):
        ...
        mip = mip.resize((int(zoom*where.winfo_width()), int(zoom*where.winfo_height()))) # zoom according to window size col x row
        self.primary_mip_zoom = [self.size[0]/where.winfo_height(), self.size[1]/where.winfo_width()] # size row x col
        self.mip_to_show = ImageTk.PhotoImage(mip)
        where.create_image(position[0], position[1], anchor='nw', image=self.mip_to_show) # this is col x row

    def clear_mip(self, where):
        # drop the precomputed frames and blank the canvas (used on segmentation reset)
        self.mip_build_token += 1 # invalidate any background build still in flight
        self.mip_ready_token = -1
        self.mip_frames = []
        self.mip_index = 0
        where.delete('all')
    ...
    def mip_zoom(self, event, zoom_point, where):
        # zoom[0] mouse scroll button rotation, zoom[1] position row, zoom[2] position col, where = where to zoom
        zoom_step = 0.5
        if (zoom_point[0] > 0):
            if self.mask_zoom >= 5: # zoom setup = object property
                ...
                self.show_mip_frame(where, self.mip_position, self.mask_zoom)
        elif (zoom_point[0] < 0):
            if self.mask_zoom <= 1:
                ...
            else:
    ...
    def get_bg_for_segmentation(self):
        ...
        cr = (p1[0]+p0[0])/2 # centre coordinates - rows
        cc = (p1[1]+p0[1])/2 # centre coordinates - cols

        a = (p0[0]-p1[0])/2 # a axis lenght - rows
        b = (p0[1]-p1[1])/2 # b axis lenght - cols
        ...
        for i in [-2, -1, 0, 1, 2]: # +-2 slices from the one where ellipse was drawn will be evaluated
            s = min(max(s0 + i, 0), self.size[2]-1) # clip to a valid slice index (avoid out-of-range near volume ends)
            temp.append(self.data[:,:,s][mask_array == 1])
    ...
    # =============================================================================
        ...
        im_col=self.size[1] # number of cols determination
        ...
        while (growth !=0):
            tempx=[] # collect newly added voxels in plain lists - much faster than np.append per voxel (O(n) vs O(n^2))
            tempy=[]
            tempz=[]
                ...
                for j in range(0,6): # range 0-6 for the connectivity = 6
                    ...
                    try:
                        ...
                        thresh*(seed_value+self.roi_bg_value):
                            ...
                            tempx.append(x_t[j]) # bin for pixels fulfilling the condition prolonged - x coordinate
                            tempy.append(y_t[j]) # bin for pixels fulfilling the condition prolonged - y coordinate
                            tempz.append(z_t[j]) # bin for pixels fulfilling the condition prolonged - z coordinate
            ...
            cnt_sx=np.array(tempx, dtype=int) # new starting container now contains the last recorded pixels from the mask
            cnt_sy=np.array(tempy, dtype=int)
            cnt_sz=np.array(tempz, dtype=int)
...
#-------------------------------------------------------------------------------------------
    ...
    def get_out_results(self, file_path):
        # export of results - column order:
        # RC_max, RC_A<thresh>, RC_peak, CRC_max, CRC_A<thresh>, CRC_peak, diameter,
        # bg_real, bg_measured, bg_diff(%), bg_COV(%), bg position (x1,y1 top-left; x2,y2 bottom-right; z slice)
        temp = [['RC_max', 'RC_A' + str(self.seg_thresh), 'RC_peak', \
                 'CRC_max', 'CRC_A' + str(self.seg_thresh), 'CRC_peak', \
                 'diameter', 'bg_real', 'bg_measured', 'bg_diff(%)', 'bg_COV(%)', \
                 'bg_x1', 'bg_y1', 'bg_x2', 'bg_y2', 'bg_z']]
        for i in range(1, len(self.roi_RC_A)+1):
            try:
                temp.append([self.roi_RC_max[i], self.roi_RC_A[i], self.roi_RC_peak[i], \
                             self.roi_CRC_max[i], self.roi_CRC_A[i], self.roi_CRC_peak[i], \
                             self.roi_obj_size[i].get()])
            except:
                tk.messagebox.showwarning("Warning", 'Segmentation of ROI ' + str(i) + ' is missing!')
                return

        # background ROI corner coords (original-image voxels, x = col, y = row) and slice z; blank if not set
        bg = self.roi_position.get('bg')
        try:
            bx1, by1 = int(round(bg[0][1])), int(round(bg[0][0]))
            bx2, by2 = int(round(bg[1][1])), int(round(bg[1][0]))
            bz = int(round(bg[0][2]))
        except (TypeError, IndexError):
            bx1 = by1 = bx2 = by2 = bz = ''

        # background parameters live on their own row with all RC_/CRC_/diameter cells empty,
        # so the bg values align under bg_real, bg_measured, bg_diff(%), bg_COV(%) and the bg position columns
...
class mainWindow:
    def __init__(self, master):
        ...
        # rotating MIP animation state (variant B) - the playback just cycles precomputed frames
        self.spin_running = False
        self.spin_after_id = None
        self.spin_interval = 100 # ms between frames (~10 fps) - CPU cost is negligible, machine independent

        self.bg_start = None # start point of the background-ROI drag (set on press, consumed on release)

        # background-position manual entry - five fields = top-left (x1,y1) and bottom-right (x2,y2)
        # corners of the square in which the background circle is inscribed, plus a single slice (z),
        # since the background ROI is always drawn on one slice. Original-image voxel coordinates
        # (x = column, y = row, z = slice). Their values survive are store for re-opening different data
        # or could be inserted manually
        self.bg_coord_vars = {k: tk.StringVar() for k in ('x1', 'y1', 'x2', 'y2', 'z')}

        self.plus = tk.PhotoImage(file = r"plus.png") # open icon files - plus a minus pro změnu prahu zobrazení
        ...
        self.frame_h_i = tk.Frame(master)
        self.frame_h = tk.LabelFrame(self.frame_h_i, text='Image threshold')
        self.frame_i = tk.LabelFrame(self.frame_h_i, text='Background position')
        ...
        # single rotating front-view MIP - spin/stop button
        self.frame_b.grid_columnconfigure(0, weight=1) # keep the canvas and the button centered in the column
        self.mip_view = tk.Canvas(self.frame_b, width=self.Xsize/2.5, height=self.Ysize/2.5, borderwidth=0, bg='white')
        self.mip_view.config(highlightbackground='black', highlightthickness='1')
        self.mip_view.grid(row=0, column=0)

        self.spin_btn = tk.Button(self.frame_b, text='Spin', width=8, command=self.toggle_spin)
        self.spin_btn.grid(row=1, column=0, pady=2)
        ...
        # empty side columns absorb the extra width so the controls stay centered under the MIPs column
        self.frame_h.grid_columnconfigure(0, weight=1)
        self.frame_h.grid_columnconfigure(5, weight=1)

        self.thresh_set = tk.Label(self.frame_h, width = 10)
        self.thresh_set.grid(row=0, column=1, sticky='nsew')
        self.thresh_set['text'] = '1'
        label = tk.Label(self.frame_h, text=' image threshold', anchor='sw', justify='left').grid(row=0, column=2, sticky='nsew')
        ...
        # =============================================================================
        # frame_i - Background position (manual entry / recall of the bg ROI square corners)
        # =============================================================================
        # empty side columns absorb extra width so the fields stay centered under the MIPs column
        self.frame_i.grid_columnconfigure(0, weight=1)
        self.frame_i.grid_columnconfigure(5, weight=1)

        tk.Label(self.frame_i, text='x').grid(row=0, column=2)
        tk.Label(self.frame_i, text='y').grid(row=0, column=3)

        tk.Label(self.frame_i, text='top-left', anchor='w').grid(row=1, column=1, sticky='w')
        tk.Entry(self.frame_i, textvariable=self.bg_coord_vars['x1'], width=6, exportselection=0).grid(row=1, column=2)
        tk.Entry(self.frame_i, textvariable=self.bg_coord_vars['y1'], width=6, exportselection=0).grid(row=1, column=3)

        tk.Label(self.frame_i, text='bottom-right', anchor='w').grid(row=2, column=1, sticky='w')
        tk.Entry(self.frame_i, textvariable=self.bg_coord_vars['x2'], width=6, exportselection=0).grid(row=2, column=2)
        tk.Entry(self.frame_i, textvariable=self.bg_coord_vars['y2'], width=6, exportselection=0).grid(row=2, column=3)

        # a single slice (z) field - the background ROI is always drawn on one slice
        tk.Label(self.frame_i, text='slice (z)', anchor='w').grid(row=3, column=1, sticky='w')
        tk.Entry(self.frame_i, textvariable=self.bg_coord_vars['z'], width=6, exportselection=0).grid(row=3, column=2)

        tk.Button(self.frame_i, text='Use this background coordinates', command=self.apply_bg_coords).grid(row=4, column=0, columnspan=6, sticky='ew', pady=(3, 0))
        ...
        self.frame_h_i.grid(row=1, column=2, columnspan=1, sticky='new')
        self.frame_h_i.grid_columnconfigure(0, weight=1)
        self.frame_h.grid(row=0, column=0, sticky='ew')
        self.frame_i.grid(row=1, column=0, sticky='ew', pady=(4, 0))
        frame_switch(self.frame_h, 0) # inactive until data opened - frame_switch = function to activate/deactivate frames
        frame_switch(self.frame_i, 0) # inactive until data opened

    def open_data(self):
        # preserve the manually entered lesion diameters across a re-open 
        prev_diam = {}
        if getattr(self, 'phantom', None) is not None:
            for k, v in self.phantom.roi_obj_size.items():
                try:
                    prev_diam[k] = v.get()
                except Exception:
                    pass
        ...
        # stop any rotation left over from a previous dataset and blank the MIP until something is segmented
        self.stop_spin()
        self.phantom.clear_mip(self.mip_view)
        ...
        self.mip_view.bind("<Shift-MouseWheel>", self.mask_zoom)
        self.mip_view.bind("<ButtonPress-2>", self.mip_click_reset)
        ...
        # restore the preserved lesion diameters into the freshly created IntVars
        for k, val in prev_diam.items():
            if k in self.phantom.roi_obj_size:
                try:
                    self.phantom.roi_obj_size[k].set(val)
                except Exception:
                    pass

        # recall a previously used background ROI: its corner coords survive in the entry fields, so if all
        # five are filled, rebuild the ROI (staying on the current/central slice) and - when the experiment
        # information is available - recompute the background
        if all(self.bg_coord_vars[k].get().strip() for k in ('x1', 'y1', 'x2', 'y2', 'z')):
            self.apply_bg_coords(jump=False, announce=False)
    ...
    def rois_pos_save(self, i):
        ...
        # rebuild the rotating MIP frames on a background thread (variant B) so segmentation is not held up;
        # the spin (re)starts automatically once the new frames are ready - already after a single source
        self.rebuild_mip_async()
        # get the result
    ...
    def threshold_img(self, i):
        ...
        if self.phantom.thresh <= 1: self.phantom.thresh = 1
    ...
    def mask_zoom(self, event):
        zoom = [event.delta, event.x, event.y]
        self.phantom.mip_zoom(self, zoom, self.mip_view)

    def mip_click_reset(self, event):
        # middle-click resets the MIP zoom/pan back to default, same as on the phantom display
        self.phantom.mask_zoom = 1
        self.phantom.mip_position = [0, 0]
        self.phantom.show_mip_frame(self.mip_view, self.phantom.mip_position, self.phantom.mask_zoom)

    # =============================================================================
    # Background position - manual entry / recall of the background ROI
    # =============================================================================
    def update_bg_coord_fields(self):
        # write the current background-ROI corner coords into the entry fields, in original-image
        # voxel coordinates (x = col, y = row) plus the single slice (z), whole voxels in original PET image.
        # The internal store roi_position['bg'] is a flat ((r,c,s),(r,c,s)) - s is the same slice
        bg = self.phantom.roi_position.get('bg')
        try:
            p0, p1 = bg[0], bg[1]
        except (TypeError, IndexError):
            return
        self.bg_coord_vars['x1'].set(str(int(round(p0[1])))) # col
        self.bg_coord_vars['y1'].set(str(int(round(p0[0])))) # row
        self.bg_coord_vars['x2'].set(str(int(round(p1[1]))))
        self.bg_coord_vars['y2'].set(str(int(round(p1[0]))))
        self.bg_coord_vars['z'].set(str(int(round(p0[2]))))  # single slice

    def apply_bg_coords(self, jump=True, announce=True):
        # (re)build the background ROI from the entered / recalled corner fields (original-image voxel
        # coords, x = col, y = row) on the given slice (z). Draws the ROI and - when the experiment
        # information is filled - recomputes the background data/
        if getattr(self, 'phantom', None) is None:
            return
        try:
            x1 = int(round(float(self.bg_coord_vars['x1'].get().replace(',', '.'))))
            y1 = int(round(float(self.bg_coord_vars['y1'].get().replace(',', '.'))))
            x2 = int(round(float(self.bg_coord_vars['x2'].get().replace(',', '.'))))
            y2 = int(round(float(self.bg_coord_vars['y2'].get().replace(',', '.'))))
            z = int(round(float(self.bg_coord_vars['z'].get().replace(',', '.'))))
        except (ValueError, AttributeError):
            if announce:
                tk.messagebox.showwarning("Warning", "Fill all background coordinates with numbers!")
            return

        # the coordinates must lie inside the loaded matrix: x (col) in [0, cols-1], y (row) in [0, rows-1],
        # z (slice) in [0, slices-1]. Ranges come from the data - general condition
        rows, cols, slices = self.phantom.size[0], self.phantom.size[1], self.phantom.size[2]
        if not (0 <= x1 < cols and 0 <= x2 < cols and 0 <= y1 < rows and 0 <= y2 < rows and 0 <= z < slices):
            if announce:
                tk.messagebox.showwarning("Warning",
                    "Background coordinates are outside the loaded data.\n"
                    "Valid ranges (whole voxels): x 0-%d, y 0-%d, slice 0-%d." % (cols-1, rows-1, slices-1))
            return

        # reject a degenerate square (single point / zero-size ellipse) - same rule as get_bg_for_segmentation
        if x1 == x2 or y1 == y2:
            if announce:
                tk.messagebox.showwarning("Warning", "Background square is degenerate (zero width or height)!")
            return

        # reconstruct the flat ((r,c,s),(r,c,s)) pair used internally (r = row = y, c = col = x, s = slice = z)
        self.phantom.roi_position['bg'] = ((y1, x1, z), (y2, x2, z))
        self.phantom.roi_bg[0] = 1 # mark the background ROI as existing (non-zero flag)

        if jump: # show the slice the background lives on so the ROI is visible
            self.phantom.slice = min(max(z, 0), self.phantom.size[2]-1)
            self.phantom_slice_lbl['text'] = 'slice: ' + str(self.phantom.slice)
            self.phantom.show_me_slices(self.phantom.data, self.phantom.slice, self.phantom_slice_view, self.phantom.position, self.phantom.zoom, self.phantom.thresh)
        self.bg_roi_redraw(self.phantom_slice_view)

        # compute the background statistics immediately when the experiment information is available
        if not ((self.data_srcs_act.get() == '0') or (self.data_srcs_time.get() == 'hhmmss') or
                (self.data_bcg_act.get() == '0') or (self.data_bcg_time.get() == 'hhmmss')):
            self.all_setup_fill(self.phantom.all_setup)
    ...
    def spin_step(self):
        if not self.spin_running:
            return
        frames = self.phantom.mip_frames # local ref - the background build may swap the list at any time
        if frames:
            self.phantom.mip_index = (self.phantom.mip_index + 1) % len(frames)
            self.phantom.show_mip_frame(self.mip_view, self.phantom.mip_position, self.phantom.mask_zoom)
        self.spin_after_id = self.master.after(self.spin_interval, self.spin_step)

    def rebuild_mip_async(self):
        # recompute the rotation frames on a background thread so segmenting a source never blocks the script
        ...
        token = self.phantom.mip_build_token
        mask = self.phantom.mask_labelled.copy() # snapshot so the worker sees a stable array
        ...
        self._poll_mip_ready(token)

    def _poll_mip_ready(self, token):
        # main-thread poll: when the worker for this token has stored its frames, show one and start spinning
        if token != self.phantom.mip_build_token:
            return # a newer build (or a reset) took over - that one drives the display now
        if self.phantom.mip_ready_token == token and self.phantom.mip_frames:
            self.phantom.show_mip_frame(self.mip_view, self.phantom.mip_position, self.phantom.mask_zoom)
            self.start_spin()
            return
        self.master.after(150, lambda: self._poll_mip_ready(token))

    def start_spin(self):
        # begin (or keep) the rotation - called once the background frames are ready
        if not self.spin_running:
            self.spin_running = True
            self.spin_btn.config(text='Stop')
            self.spin_step()

    def stop_spin(self):
        self.spin_running = False
        self.spin_btn.config(text='Spin')
        if self.spin_after_id is not None:
            self.master.after_cancel(self.spin_after_id)
            self.spin_after_id = None
    ...
    def roi_character_create(self, how_many, where, roi_size, RC_max_lbl, RC_A_lbl, RC_peak_lbl, CRC_max_lbl, CRC_A_lbl, CRC_peak_lbl, bcg_info, thresh, mode='RC'):
        ...
        # BOTH are stored switching the display mode shows the relevant without new computation
        if mode == 'CRC':
    ...
    def mouse_draw_start(self, event, on_off, roi_list):
        ...
        if on_off == 1: # start drawing the roi
            ...
            if (self.data_srcs_act.get() == '0') or (self.data_srcs_time.get() == 'hhmmss') or \
                ...
                # store the start point separately as a flat (r,c,s) triple - the end point is paired with
                # THIS value on release, so roi_position['bg'] can never nest into ((pair),point) 
                self.bg_start = self.phantom.slice_click(self, (self.y_click, self.x_click))
                self.phantom.roi_position['bg'] = self.bg_start
                self.drawn = None
            ...
            if (self.data_srcs_act.get() == '0') or (self.data_srcs_time.get() == 'hhmmss') or \
                ...
                if getattr(self, 'bg_start', None) is None:
                    return
                roi_list[0] = self.drawn
                self.x_click = event.x
                self.y_click = event.y
                # build a flat (start, end) pair from the stored start point, then consume the start
                bg_end = self.phantom.slice_click(self, (self.y_click, self.x_click))
                self.phantom.roi_position['bg'] = (self.bg_start, bg_end)
                self.bg_start = None # next ROI must begin with a fresh press
                ...
                if self.phantom.roi_position['bg'][0][0] == self.phantom.roi_position['bg'][1][0]: # single point background roi check - unintentional single click
                    ...
                    self.phantom.get_bg_for_segmentation()
                    self.update_bg_coord_fields() # keep the Background position fields in sync with the drawn ROI
    ...
    def current_mode(self):
        # which coefficient is currently displayed
        return 'CRC' if self.disp_CRC_var.get() == 1 else 'RC'

    def active_limits(self):
        # boundary-selection StringVars belonging to the currently displayed
    ...
    def reset_segmentation(self):
        ...
        self.phantom.mask_labelled = np.zeros([self.phantom.size[0], self.phantom.size[1], self.phantom.size[2]])
        GraphRC(self.frame_c, (self.Xsize, self.Ysize), [0,0], self.phantom.seg_thresh, [[],[]], [[],[]], [[],[]], self.current_mode(), self.RC[0], self.RC[1], self.RC[2], self.config, *self.active_limits())
        # stop the rotation and blank the MIP - there is nothing segmented to show anymore
        self.stop_spin()
        self.phantom.clear_mip(self.mip_view)
