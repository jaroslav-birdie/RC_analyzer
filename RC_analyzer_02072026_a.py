...
#-------------------------------------------------------------------------------------------
    ...
    def get_out_results(self, file_path):
        ...
        # bg_real, bg_measured, bg_diff(%), bg_COV(%), bg position (x1,y1,z1 top-left; x2,y2,z2 bottom-right)
        temp = [['RC_max', 'RC_A' + str(self.seg_thresh), 'RC_peak', \
                 ...
                 'bg_x1', 'bg_y1', 'bg_z1', 'bg_x2', 'bg_y2', 'bg_z2']]
        ...
        # background ROI corner coords (original-image voxels, x = col, y = row, z = slice); blank if not set
        bg = self.roi_position.get('bg')
        try:
            bx1, by1, bz1 = int(round(bg[0][1])), int(round(bg[0][0])), int(round(bg[0][2]))
            bx2, by2, bz2 = int(round(bg[1][1])), int(round(bg[1][0])), int(round(bg[1][2]))
        except (TypeError, IndexError):
...
class mainWindow:
    def __init__(self, master):
        ...
        # background-position manual entry - six fields = top-left (x1,y1,z1) and bottom-right (x2,y2,z2)
        # corners of the square in which the background circle is inscribed, in ORIGINAL-image voxel
        # coordinates (x = column, y = row, z = slice). These widgets live on the window, so their values
        # survive re-opening data and let the same background region be recalled / typed in manually.
        self.bg_coord_vars = {k: tk.StringVar() for k in ('x1', 'y1', 'z1', 'x2', 'y2', 'z2')}
        ...
        tk.Label(self.frame_i, text='z (slice)').grid(row=0, column=4)
        ...
        tk.Entry(self.frame_i, textvariable=self.bg_coord_vars['z1'], width=6, exportselection=0).grid(row=1, column=4)
        ...
        tk.Entry(self.frame_i, textvariable=self.bg_coord_vars['z2'], width=6, exportselection=0).grid(row=2, column=4)

        tk.Button(self.frame_i, text='Use this background coordinates', command=self.apply_bg_coords).grid(row=3, column=0, columnspan=6, sticky='ew', pady=(3, 0))
    ...
    def open_data(self):
        ...
        if all(self.bg_coord_vars[k].get().strip() for k in ('x1', 'y1', 'z1', 'x2', 'y2', 'z2')):
            self.apply_bg_coords(jump=False)
    ...
    def update_bg_coord_fields(self):
        # write the current background-ROI corner coords into the six entry fields, in original-image
        # voxel coordinates (x = col, y = row, z = slice), rounded to whole voxels. The internal store
        # roi_position['bg'] is a flat ((r,c,s),(r,c,s)) pair. Called after every manual (re)draw.
        ...
        self.bg_coord_vars['z1'].set(str(int(round(p0[2])))) # slice
        self.bg_coord_vars['x2'].set(str(int(round(p1[1]))))
        self.bg_coord_vars['y2'].set(str(int(round(p1[0]))))
        self.bg_coord_vars['z2'].set(str(int(round(p1[2]))))

    def apply_bg_coords(self, jump=True):
        # (re)build the background ROI from the six entered / recalled corner fields (original-image
        # voxel coords, x = col, y = row, z = slice). Draws the ROI and - when the experiment information
        # is filled - recomputes the background statistics right away (same as a manual drag).
        ...
        try:
            ...
            z1 = int(round(float(self.bg_coord_vars['z1'].get().replace(',', '.'))))
            x2 = int(round(float(self.bg_coord_vars['x2'].get().replace(',', '.'))))
            y2 = int(round(float(self.bg_coord_vars['y2'].get().replace(',', '.'))))
            z2 = int(round(float(self.bg_coord_vars['z2'].get().replace(',', '.'))))
        except (ValueError, AttributeError):
            tk.messagebox.showwarning("Warning", "Fill all six background coordinates with numbers!")
            return
        ...
        self.phantom.roi_position['bg'] = ((y1, x1, z1), (y2, x2, z2))
        ...
        if jump: # show the slice the background lives on so the ROI is visible
            self.phantom.slice = min(max(z1, 0), self.phantom.size[2]-1)
            self.phantom_slice_lbl['text'] = 'slice: ' + str(self.phantom.slice)
            self.phantom.show_me_slices(self.phantom.data, self.phantom.slice, self.phantom_slice_view, self.phantom.position, self.phantom.zoom, self.phantom.thresh)
        self.bg_roi_redraw(self.phantom_slice_view)
