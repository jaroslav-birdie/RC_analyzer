...
# =============================================================================
        ...
        # adaptive in-plane downsampling chosen from the matrix size (rows x cols): keep full resolution for
        ...
        if inplane >= 400:
            ...
            self.mip_downsample = 1   # <200: no downsampling needed
...
class mainWindow:
    def __init__(self, master):
        ...
        # (x = column, y = row, z = slice). These widgets live on the window, so their values survive
        # re-opening data and let the same background region be recalled / typed in manually.
        ...
        # frame_h (Image threshold) and frame_i (Background position) are stacked inside one container so
        # they share the single column-2 / row-1 cell; the container hugs the top (Image threshold moves up)
        self.frame_col2 = tk.Frame(master)
        self.frame_h = tk.LabelFrame(self.frame_col2, text='Image threshold')
        self.frame_i = tk.LabelFrame(self.frame_col2, text='Background position')
        ...
        # single rotating front-view MIP (variant B) - replaces the former LEFT/UP pair; the view spins
        # about the phantom long axis by cycling precomputed rotation frames (Spin/Stop button below)
        ...
        # column-2 middle band: container hugs the top so Image threshold moves up, Background position sits below it
        self.frame_col2.grid(row=1, column=2, columnspan=1, sticky='new')
        self.frame_col2.grid_columnconfigure(0, weight=1)
    ...
    def open_data(self):
        # preserve the manually entered lesion diameters across a re-open (until now only the
        # 'Experiment information' fields survived, because they live on the window). The IEC Body
        # Phantom buttons still overwrite these on demand.
        prev_diam = {}
        ...
        # six are filled, rebuild the ROI (staying on the current/central slice) and - when the experiment
        # information is available - recompute the background statistics right away
    ...
    def update_bg_coord_fields(self):
        ...
        # voxel coordinates (x = col, y = row) plus the single slice (z), rounded to whole voxels.
        # The internal store roi_position['bg'] is a flat ((r,c,s),(r,c,s)) pair (both points share the
        # same slice). Called after every manual (re)draw.
    ...
    def apply_bg_coords(self, jump=True, announce=True):
        ...
        # information is filled - recomputes the background statistics right away (same as a manual drag).
        # jump=True (button press) shows the background slice; jump=False (re-open) keeps the current slice.
        # announce=True shows warnings for bad input; announce=False (re-open) fails silently, since the
        # stored coords may simply not fit a differently sized dataset.
        ...
        # z (slice) in [0, slices-1]. Ranges come from the data, so this works for any matrix / slice count.
    ...
    def mouse_draw_start(self, event, on_off, roi_list):
        ...
        if on_off == 1: # start drawing the roi
            ...
            if (self.data_srcs_act.get() == '0') or (self.data_srcs_time.get() == 'hhmmss') or \
                ...
                # THIS value on release, so roi_position['bg'] can never nest into ((pair),point) (caused the
                # 'float + tuple' crash in get_bg_for_segmentation)
            ...
            if (self.data_srcs_act.get() == '0') or (self.data_srcs_time.get() == 'hhmmss') or \
                ...
                # ignore a release that has no matching start press (would otherwise re-wrap an existing pair)
